...
export default function LausanaGracias() {
  ...
  // (Valmira, Vellmari, Portafolio, Loreta). Para reportar esta campaña por
  // separado hay que crear una conversión nueva en Google Ads y cambiar el
  // label aquí — no se puede hacer desde el código.
  useEffect(() => {
    if (typeof (window as any).gtag === 'function') {
      (window as any).gtag('event', 'conversion', { send_to: 'AW-17453917774/wazACPXDnMQcEM7M1oJB' });
    }
    if (typeof (window as any).fbq === 'function') {
      (window as any).fbq('track', 'Lead', { content_name: 'Lausana Residencial Landing', content_category: 'ads' });
    }
  }, []);
  ...
  return (
    <section className="relative flex min-h-[100svh] items-center justify-center overflow-hidden bg-ink px-6 py-16 text-white">
      <Image
        src="/desarrollos/olivia/10.-Parque-urbano.jpg"
        alt="Lausana Residencial, Cancún"
      ...
      <div className="relative z-10 w-full max-w-lg text-center">
        ...
        <p className="mx-auto mt-4 max-w-md text-[15px] font-light leading-relaxed text-white/70">
          Un asesor especializado en Lausana Residencial te contactará muy pronto con disponibilidad
          real, precios y planes de pago de Olivia y Loreta. Mientras tanto, adelántate por WhatsApp:
        </p>

        <a
          href={waLink('Hola, acabo de dejar mis datos en la campaña de Lausana Residencial (Olivia y Loreta). Quiero más información.')}

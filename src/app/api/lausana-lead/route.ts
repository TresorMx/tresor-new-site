...
const PROYECTO_LABELS: Record<string, string> = {
  olivia: 'Olivia Wow Condos',
  loreta: 'Loreta Wow Condos',
...
export async function POST(req: NextRequest) {
  try {
    ...
    const notes = [
      'Lausana Residencial Landing (Olivia + Loreta)',
      countryLabel ? `País: ${countryLabel}` : null,
      proyectoLabel ? `Proyecto: ${proyectoLabel}` : null,
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        await resend.emails.send({
          ...
          subject: `🏡 Nuevo lead Lausana Residencial (${channel}) — ${firstName}`,
          html: `
            <div style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:540px;margin:0 auto;color:#0E0E0E;">
              <div style="border-bottom:2px solid #FAB413;padding-bottom:12px;margin-bottom:20px;">
                <h2 style="margin:0;font-size:20px;">🏡 Nuevo lead · Lausana Residencial (Olivia + Loreta)</h2>
              </div>
              <table style="width:100%;border-collapse:collapse;font-size:14px;">
                ...
                <tr><td style="padding:8px 0;color:#6B6863;">Proyecto</td><td style="text-align:right;font-weight:600;">${proyectoLabel ?? '—'}</td></tr>
        ...
        console.error('[lausana-lead] email error', emailErr);
      }
    }

    return NextResponse.json({ ok: true, contactId: ghlRes.contactId ?? null });
  } catch (e) {
    console.error('[lausana-lead]', e);

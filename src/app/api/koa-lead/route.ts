...
const TIPOLOGIA_LABELS: Record<string, string> = {
  '1rec-studio': '1 Recámara + Studio',
  '2rec-out': '2 Recámaras Out',
  '3rec': '3 Recámaras',
...
export async function POST(req: NextRequest) {
  try {
    ...
    // generar por el chatbot de la landing, la ficha o cualquier otra landing.
    const [ghlRes] = await Promise.all([
      upsertGHLContact({
        ...
        phone,
        source: 'agenda',
        tags: ['Tresor Web', 'koa-onix', 'Koa Landing', channel],
        customFields: {
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        await resend.emails.send({
          ...
          subject: `🏠 Nuevo lead Koa (${channel}) — ${firstName}`,
          html: `
            <div style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:540px;margin:0 auto;color:#0E0E0E;">
              <div style="border-bottom:2px solid #FAB413;padding-bottom:12px;margin-bottom:20px;">
                <h2 style="margin:0;font-size:20px;">🏠 Nuevo lead · Koa Landing</h2>
              </div>
        ...
        console.error('[koa-lead] email error', emailErr);
      }
    }

    return NextResponse.json({ ok: true, contactId: ghlRes.contactId ?? null });
  } catch (e) {
    console.error('[koa-lead]', e);
